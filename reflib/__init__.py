...
def search_ref(paper_name, debug=False):
    json_request = search_ref_json(paper_name, debug)
...
def search_ref_json(paperName, debug=False):
    if(debug): print("request paper html")
    page = page_request(paperName)
